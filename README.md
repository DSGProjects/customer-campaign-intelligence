...
├── assets/
│   ├── Dashboard.png
│   ├── tasa_respuesta_segmento.png
│   └── campanas_aceptadas_segmento.png
│
├── data/
│   ├── raw/
...
| `Segmento_Valor` | Clasificación Bajo / Medio / Alto según percentiles de `Gasto_Total` |
...
El uso del canal de catálogo presenta una diferencia considerable entre clientes de alto y bajo valor: en promedio, **5,63 compras por catálogo en el segmento Alto frente a 0,28 en el segmento Bajo**, una diferencia de aproximadamente **20 veces**.
...
# Exploración en Python

Antes de construir el dashboard, el análisis exploratorio se realizó en Python (Pandas, Matplotlib y Seaborn), disponible completo en `notebooks/04_analysis.ipynb`. Estas dos vistas respaldan los dos primeros insights:

![Tasa de respuesta por segmento de valor](assets/tasa_respuesta_segmento.png)

*Tasa de respuesta por segmento de valor — respalda el Insight 1.*

![Campañas aceptadas por segmento y respuesta](assets/campanas_aceptadas_segmento.png)

*Promedio de campañas aceptadas previamente, por segmento y respuesta actual — respalda el Insight 2.*
...
- Los botones **Bajo / Medio / Alto** filtran las tarjetas de KPIs (Total Clientes, Tasa de respuesta, Gasto Promedio) y el gráfico de dispersión de Ingresos vs. Gasto Total.
- Los gráficos de **Tasa de respuesta** e **Historial de campañas por segmento y respuesta** muestran siempre los tres segmentos completos, ya que su propósito es la comparación entre ellos.

---

# Limitaciones

- El dataset es de carácter público y educativo (no proviene de una empresa real), por lo que los hallazgos deben entenderse como una demostración del enfoque analítico, no como resultados aplicables directamente a un negocio.
- El tamaño de muestra (2.236 clientes) y el periodo temporal cubierto son limitados, por lo que no es posible confirmar si los patrones identificados se sostienen en el tiempo.
- La segmentación por valor usa percentiles (33 % y 66 %) sobre `Gasto_Total`; es una elección metodológica razonable, pero no la única forma válida de segmentar clientes.
